...
Man beachte, dass diese Symmetrien $T\to T$ nicht identisch sind mit den Symmetrien $Trans(\Lambda)\to Trans(\Lambda)$. Ist etwa $g$ eine Schraubung, so ist $R_g: Trans(\Lambda) \to Trans(\Lambda)$ eine Drehung. Hingegen operieren Schraubungen und Drehungen identisch auf $T$.

Dies ist ein Beispiel dafür, dass dieselbe Gruppe $\Aut(\Lambda) / Trans(\Lambda)$ nicht nur auf eine Weise als Symmetriegruppe auftreten kann. Dieselbe Gruppe kann auch verschiedenen Objekten (hier etwa: Dem Translationsgitter und dem 3-Torus) auf völlig verschiedene Arten operieren.
\end{remark}
